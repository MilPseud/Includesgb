...
if(isset($_POST['valid']))
{
   ...
   $malus5 = htmlspecialchars($_POST['malus5']);
	if(!empty($_POST['lateralite']) AND !empty($_POST['poids']) AND !empty($_POST['taille']) AND !empty($_POST['Physique']) AND !empty($_POST['Mental']))
	{
		$nomlength = strlen($nom);
		if($nomlength <= 225)
		{
			$updateuser = $db->prepare('UPDATE member_list SET nom = ?, pseudo = ? WHERE id = ?');
			$updateuser->execute(array($nom, $mc, $ID));
			...
			$updatealt = $db->prepare('UPDATE div_alert SET Editperso = ? WHERE id = ?');
...
<form action="index.php?p=editprofile&perso=<?= $perso?>" method="POST">
				...
				<td valign="top" width="50%">
					<table class="pnjtable" style="border-radius: 50px;" cellspacing="5" cellpadding="5" align="center">
						<tbody>
								...
								<span class="username-detail" style="color: <?=$color?>"><?= $userinfo['username']?>
											</span>
										<br>
										<br>
